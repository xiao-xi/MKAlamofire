...
public enum MKRequestCacheDomain: Int {
    case expired = -1
...
/// 网络请求的子类
/// 缓存的网络数据默认放在.../Library/{MKConfig.shared.cacheFileName}/..目录下
///  MKRequest is the base class you should inherit to create your own request class.
///  Based on MKBaseRequest, MKRequest adds local caching feature. Note download
...
///  And the cache data will save in .../Library/{MKConfig.shared.cacheFileName}/..
...
///=============================================================================
    ...
    private var _cacheMetadata: MKMetadata?
    private let MKRequestCahceErrorDomain = "com.MKAgent.request.cache"
    
// MARK: - Override MKBaseRequest Response
    
///=============================================================================
/// @name Override MKBaseRequest Response
///=============================================================================
    ...
    open func saveResponseDataToCacheFile(_ data:Data?) {
        if self.cacheInSeconds > 0 && !self.isDataFromCache {
            if let data = data, data.count > 0 {
                // cache
                do {
                    try data.write(to: URL(fileURLWithPath: MKCache.shared.cacheFilePath(self)), options: .atomic)
                } catch let reason {
                    MKLog("Save cache failed, reason: \"\(reason.localizedDescription)\"")
                }
                let metadata = MKMetadata()
                metadata.version = self.cacheVersion
                metadata.sensitiveDataString = self.cacheSensitiveData?.description
                metadata.stringEncoding = MKUtils.stringEncodingFromRequest(self)
                metadata.createDate = Date()
                metadata.appVersionString = MKUtils.appVersion
                NSKeyedArchiver.archiveRootObject(metadata, toFile: MKCache.shared.cacheMetadataFilePath(self))
            }
        }
    }
...
///=============================================================================
    ...
    private func loadCacheWithError(_ error: inout Error? ) -> Bool {
        ...
        if self.cacheInSeconds < 0 {
            if error != nil {
                error = NSError(domain: MKRequestCahceErrorDomain, code: MKRequestCacheDomain.invalidCacheTime.rawValue, userInfo: [NSLocalizedDescriptionKey:"Invalid cache time."])
            }
            return false
        }
        ...
        if !self.loadCacheMetadata {
            if error != nil {
                error = NSError(domain: MKRequestCahceErrorDomain, code: MKRequestCacheDomain.invalidMetadata.rawValue, userInfo: [NSLocalizedDescriptionKey:"Invalid metadata. may be cache not exists."])
            }
            return false
        }
        ...
        if !self.loadCacheData {
            if error != nil {
                error = NSError(domain: MKRequestCahceErrorDomain, code: MKRequestCacheDomain.invalidCacheData.rawValue, userInfo: [NSLocalizedDescriptionKey:"Invalid cache data."])
            }
            return false
        }
    ...
    private func validateCacheWithError(_ error: inout Error? ) -> Bool {
        ...
        if let cacheDate = _cacheMetadata?.createDate {
            ...
            if duration < 0 || duration > self.cacheInSeconds {
                if error != nil {
                    error = NSError(domain: MKRequestCahceErrorDomain, code: MKRequestCacheDomain.expired.rawValue, userInfo: [NSLocalizedDescriptionKey:"Cache expired."])
                }
                return false
            }
        }
        ...
        if let version = _cacheMetadata?.version {
            if version != self.cacheVersion {
                if error != nil {
                    error = NSError(domain: MKRequestCahceErrorDomain, code: MKRequestCacheDomain.versionMismatch.rawValue, userInfo: [NSLocalizedDescriptionKey:"Cache version misMatch."])
                }
                return false
            }
        }
        ...
        if currentSensitiveString != nil || sensitiveString != nil {
            if currentSensitiveString?.count != sensitiveString?.count || currentSensitiveString != sensitiveString {
                if error != nil {
                    error = NSError(domain: MKRequestCahceErrorDomain, code: MKRequestCacheDomain.versionMismatch.rawValue, userInfo: [NSLocalizedDescriptionKey:"Cache sensitive data misMatch."])
                }
                return false
            }
        }
        ...
        let currentAppVersion = MKUtils.appVersion
        if appVersion != nil || currentAppVersion != nil {
            if appVersion?.count != currentAppVersion?.count || appVersion != currentAppVersion {
                if error != nil {
                    error = NSError(domain: MKRequestCahceErrorDomain, code: MKRequestCacheDomain.appVersionMismatch.rawValue, userInfo: [NSLocalizedDescriptionKey:"App version misMatch."])
                }
                return false
            }
        }
...
// MARK: - Cache
    ...
    private var loadCacheMetadata: Bool {
        let path = MKCache.shared.cacheMetadataFilePath(self)
        let manager = FileManager.default
        if manager.fileExists(atPath: path, isDirectory: nil) {
            
            if let meta = NSKeyedUnarchiver.unarchiveObject(withFile: path) as? MKMetadata {
                _cacheMetadata = meta
                return true
            }
            MKLog("Load cache metadata failed.")
            return false
        }
        return false
    }
    ...
    private var loadCacheData: Bool {
        let path = MKCache.shared.cacheFilePath(self)
        let manager = FileManager.default
        ...
        if manager.fileExists(atPath: path, isDirectory: nil) {
            do {
                ...
                MKLog("Load cache Data is failed, reason: \"\(reason.localizedDescription)\"")
                return false
            }
        }
        return false
