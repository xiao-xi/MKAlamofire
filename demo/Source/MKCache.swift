...
/// MKCache is a cache handler class
public struct MKCache {
    
    public static let shared = MKCache()
...
///=============================================================================
    ...
    ///   - complete: when remove completed, closure will be used. if not set, the files will remove in background.
    public func removeCacheFiles(for request: MKBaseRequest? = nil, complete: CacheSizeCleanClosure? = nil) {
...
///=============================================================================
    ...
    public func cacheFilePath(_ request: MKBaseRequest) -> String {
    ...
    public func cacheMetadataFilePath(_ request: MKBaseRequest) -> String {
...
///=============================================================================
    ...
    private func cacheFileName(_ request: MKBaseRequest) -> String {
        let requestURL = request.requestURL
        let baseURL = MKConfig.shared.baseURL
        ...
        let cacheFileName = MKUtils.md5WithString(requestInfo)
        return cacheFileName
    }
    
    /// The path of the cache file
    private func cacheBasePath(_ request: MKBaseRequest) -> String {
        let path = NSSearchPathForDirectoriesInDomains(.cachesDirectory, .userDomainMask, true).first!
        var cachePath = (path as NSString).appendingPathComponent(MKConfig.shared.cacheFileName)
        
        // Filter cache dirPath if needed
        let filters = MKConfig.shared.cacheDirPathFilters
        if !filters.isEmpty {
    ...
    private func createBaseCachePath(_ path: String) {
        do {
            ...
            MKUtils.addNotBackupAttribute(path)
        } catch let reason {
            MKLog("Create cache directory failed, reason = \"\(reason.localizedDescription)\"")
        }
    }
        ...
        DispatchQueue.wbCurrent.async {
            ...
            filePaths?.forEach {
                ...
                do {
                    ...
                    MKLog("Remove Failed! remove file failed from \(fileAbsoluePath) with reason is: \(error.localizedDescription)")
                }
            }
            DispatchQueue.main.async {
                complete?(true)
